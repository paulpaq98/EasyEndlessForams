...
def prepare_trainings(df, num_fold, data_type, batch_size, transforms_dict, num_workers):
    ...
    for fold_number in range(max_fold):
        ...
        # Dictionnaire
        training_dict[f"Kfold_{num_fold:02d}"]['train_loader'] = train_loader
        training_dict[f"Kfold_{num_fold:02d}"]['val_loader'] = val_loader
        training_dict[f"Kfold_{num_fold:02d}"]['class_names'] = class_names
        training_dict[f"Kfold_{num_fold:02d}"]['num_classes'] = num_classes
        training_dict[f"Kfold_{num_fold:02d}"]['criterion'] = criterion
        training_dict[f"Kfold_{num_fold:02d}"]['fold_id'] = fold_number
        training_dict[f"Kfold_{num_fold:02d}"]['num_fold'] = num_fold
